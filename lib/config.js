...
export async function getTelefonoSoporte() {
  const { data } = await supabase.from('config_app').select('valor').eq('clave', 'telefono_soporte').maybeSingle();
  return data ? data.valor : null;
}

export async function setTelefonoSoporte(telefono) {
  await supabase.from('config_app').upsert({ clave: 'telefono_soporte', valor: telefono });
}
